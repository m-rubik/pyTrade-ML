...
def do_ml(ticker,tickerFile="./tickers/sp500tickers.pickle",hm_days=7,testFraction=0.25,initialQuantity=100):
    ...
    initialbalance_in_securities = df_test.loc[starting_date,ticker] * initialQuantity
    ...
    timeFormat = '%Y-%m-%d'
    elapsedTime = str(datetime.strptime(index, timeFormat) - datetime.strptime(starting_date, timeFormat))
    ...
    print('Initial Price per Share:',str(round(df.loc[starting_date, ticker],2)),'$')
    print('Initial Buy-In Cost on',starting_date,':',round(initialbalance_in_securities,2),'$')
    ...
    days = (datetime.strptime(index, timeFormat) - datetime.strptime(starting_date, timeFormat)).days
    years = days/365.25
...
def trackTicker(ticker,initialQuantity,starting_date,tickerFile="./tickers/sp500tickers.pickle"):
    ...
    myDict = {ticker:initialQuantity}
    initialbalance_in_securities = df.loc[starting_date,'5. adjusted close'] * initialQuantity
    # myAccount = accounts.Account(-1*initialbalance_in_securities,initialbalance_in_securities,myDict,4.95)
    ...
    timeFormat = '%Y-%m-%d'
    elapsedTime = str(datetime.strptime(index.strftime("%Y-%m-%d"), timeFormat) - datetime.strptime(starting_date, timeFormat))
    ...
    print("Broker fee per trade:",str(myAccount.broker_fee),'$')
    print('Initial Shares Held:',str(initialQuantity))
    print('Initial Price per Share:',str(round(df.loc[starting_date,'5. adjusted close'],2)),'$')
    print('Initial Buy-In Cost on',starting_date,':',round(initialbalance_in_securities,2),'$')
    ...
    days = (datetime.strptime(index.strftime("%Y-%m-%d"), timeFormat) - datetime.strptime(starting_date, timeFormat)).days
    years = days/365.25
    average_yearly_growth = round(percentGrowth / years, 2)
    months = years * 12
    average_monthly_growth = round(percentGrowth / months, 2)
